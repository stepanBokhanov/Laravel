(function($) {
            ...
            success: function (response) {
                ...
                if (response.status == "success") {
                    if (response.action == "redirect") {
                        ...
                        if (typeof response.message != "undefined") {
                            var nav = $(opt.container).find("#alert");
                            if (nav.length) {
                                var contentNav = nav.offset().top;
                                $('html,body').animate({scrollTop: contentNav-150},'slow');
    ...
    $.ajaxModal = function(selector, url, onLoad) {
        ...
        $(selector).on('hidden.bs.modal', function () {
            ...
            $(this).find('.modal-footer').html('<button type="button" data-dismiss="modal" class="btn dark btn-outline btn-default">Close</button>');
